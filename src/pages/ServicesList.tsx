...
export function ServicesList() {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <div className="bg-white/40 px-6 md:px-12 pt-12 pb-4 sticky top-0 z-20 shadow-sm border-b border-gray-200/50 backdrop-blur-md">
        <div className="flex justify-between items-center mb-6">
          <button onClick={() => navigate(-1)} className="p-2 -ml-2 hover:bg-white rounded-full transition">
        ...
        <div className="bg-white rounded-2xl flex items-center px-4 mb-4 shadow-sm border border-gray-100 md:max-w-md">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={cn(
                ...
                activeTab === cat.id ? "bg-primary text-white border-primary shadow-md shadow-primary/20" : "bg-white text-slate-600 border-gray-200 hover:border-primary/50"
              )}
            >
              {cat.name}
            </button>
        ...
        {filteredServices.length > 0 ? (
          filteredServices.map((service, i) => (
            <motion.div 
              ...
              className="bg-white/80 backdrop-blur-sm rounded-[24px] p-3 shadow-sm border border-gray-100 flex items-center space-x-4 active:scale-[0.98] transition cursor-pointer hover:shadow-md hover:bg-white md:p-5 md:flex-col md:items-start md:space-x-0 cursor-pointer"
            >
